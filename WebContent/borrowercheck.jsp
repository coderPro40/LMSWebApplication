...
<%
	...
	String[] bookColumns = { "Title", "Branch", "Check-In", "Check-Out" };
	...
	if (request.getAttribute("getPageNoBc") != null) {
		...
		bookC = service.readBookCopiesLimitSearchOverZero(1, null);
	}

	Integer totalAuthors = service.getBookCopiesCountOverZero();
...
<div class="container">
	<div class="row">
		<div class="col-lg-12 text-center">
			<h4 class="service-heading">List of all Available Book's records</h4>
			<p id="demo"></p>
		</div>
	</div>
	<br>
	<div class="row">
		...
		<div class="col-lg-6 text-right" style="display:inline;">
			<button
				onclick="window.location='borrowerentry.jsp'"
				type="button" class="btn btn-success">Log-out</button>
		</div>
	</div>
		...
		<div class="col-md-12">
			<table class="table table-striped">
				...
				<tr>
					...
					<td><button
							onclick="this.disabled=true; window.location='checkInBorrower?bookId=<%=b.getBook().getBookId()%>&branchId=<%=b.getBranch().getBranchId() %>&cardNo=<%=request.getAttribute("cardNo") %>';"
							type="button" class="btn btn-primary">Check-In</button></td>
					<td><button onclick="<%bookId = b.getBook().getBookId(); branchId = b.getBranch().getBranchId();%>'"
							type="button" data-toggle="modal" data-target="#exampleModal"
							class="btn btn-secondary">Check-Out</button></td>
				</tr>
				<%
				count++;
					}
			...
			<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog"
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">You're about to check-out this book.
							Proceed?</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-danger"
								onclick="this.disabled=true; window.location='checkOutBorrower?bookId=<%=bookId%>&branchId=<%=branchId %>&cardNo=<%=cardNo %>';"
								>Yes</button>
							<button type="button"
